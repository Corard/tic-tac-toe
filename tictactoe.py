...
class TicTacToe(QWidget):
    def __init__(self):
        ...
        self.reset_button.setStyleSheet(
            "font-size: 16px; background-color: #ffd43b; color: black;"
        )
        ...
        if self.board[row][col] == "":
            ...
            if self.current_player == "X":
                self.buttons[row][col].setStyleSheet(
                    "color: #306998; font-size: 24px;"
                )  # Blue for X
                self.buttons[row][col].setText("X")
            else:
                self.buttons[row][col].setStyleSheet(
                    "color: #ffd43b; font-size: 24px;"
                )  # Yellow for O
            ...
            if winner:
                ...
                )  # Orange for draw message
            else:
                # Switch player
                self.current_player = "O" if self.current_player == "X" else "X"
                self.turn_label.setText(f"Player {self.current_player}'s Turn")
                self.turn_label.setStyleSheet(
                    f"font-size: 18px; color: {'#306998' if self.current_player == 'X' else '#ffd43b'};"
                )
